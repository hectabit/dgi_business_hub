...
const getMerchantDetailsById = async (req, res, next) => {
	try {
		...
		// find first in merchant stag
		const merchantStag = await MerchantStag.findOne(query)
			.populate("category")
			.lean();

		// find all merchants live
		const merchant = await Merchant.findOne(query).populate("category").lean();

		const resPayload = { ...resConfig.SUCCESS };

		const isApproved = !merchantStag ? true : false;

		resPayload.merchant = {
			...merchant,
			...merchantStag,
			category: { ...merchant?.category, ...merchantStag?.category },
			isApproved,
		};

		console.log(resPayload.merchant);
...
const approveMerchantById = async (req, res, next) => {
	try {
		...
		// fetch stag data first
		const merchantStag = await MerchantStag.findOne({
			username: merchantId,
		}).lean();

		let tempMerchantStag = {
			...merchantStag,
			address: { ...merchantStag.address },
		};

		delete tempMerchantStag._id;
		delete tempMerchantStag.username;
		delete tempMerchantStag.password;
		delete tempMerchantStag.email;

		// insert data in live merchant
		await Merchant.findOneAndUpdate(
			{ username: merchantId },
			{ $set: { ...tempMerchantStag, msg: "" } }
		);

		await MerchantStag.findOneAndDelete({ username: merchantId });
...
const disapproveMerchantById = async (req, res, next) => {
	try {
		// get merchantId
		const { merchantId, msg } = req.query;
		...
		// fetch stag data first
		const merchantStag = await MerchantStag.deleteOne({
			username: merchantId,
		}).lean();
...
const changeMerchantPassword = async (req, res, next) => {
	try {
		const { merchantUsername, oldPassword, newPassword } = req.body;

		if (!merchantUsername || !oldPassword || !newPassword) {
			return res
				.status(resConfig.BAD_REQUEST.status)
				.send(resConfig.BAD_REQUEST);
		}

		const merchant = await Merchant.findOne({ username: merchantUsername })
			.select(["username", "password"])
			.lean();

		if (await bcrypt.compare(oldPassword, merchant.password)) {
		...
		console.log("error in changeMerchantPassword", error);
		return res
			.status(resConfig.SERVER_ERROR.status)
			.send(resConfig.SERVER_ERROR);
	}
...
const getUserDetails = async (req, res, next) => {
	try {
		...
		const user = await User.findOne({ refNo: refNo }).lean();

		let dg = -1;
		let coupons = -1;

		if (user) {
			dg = user.balance.toFixed(2);
		}

		const coupon = await Coupon.findOne({ refNo: refNo }).lean();
